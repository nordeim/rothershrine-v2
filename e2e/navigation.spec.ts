...
test.describe("navigation — desktop, keyboard, skip, footer", () => {
  ...
  test("keyboard nav covers primaryNav and SkipLink focuses main", async ({ page }) => {
    ...
    // Tab to SkipLink (first focusable element)
    await page.keyboard.press("Tab");
    const skipLink = page.getByRole("link", { name: /Skip to main content/i });
    await expect(skipLink).toBeFocused();

    // Activate skip link — should focus #main-content
    await skipLink.press("Enter");
    await expect(page.locator("#main-content")).toBeFocused();

    // Tab through header nav — at least Pilgrimage link should be reachable via keyboard
    // Reset to home and tab to Pilgrimage
    await page.goto("/#/");
    await page.keyboard.press("Tab"); // skip
    await page.keyboard.press("Tab"); // logo?
    // Press Tab several times until Pilgrimage is focused (tolerant: just check it can be focused)
  ...
  test("NotFound Return Home works", async ({ page }) => {
    ...
    await expect(page.getByText(/This path doesn't lead to the Shrine/i)).toBeVisible();
    await page.getByRole("link", { name: /Return Home/i }).click();
    await expect(page).toHaveURL(/#\/$|\/#\/\?/);
    await expect(page.getByRole("heading", { name: /The Shepherd Who Stayed/i })).toBeVisible();
  });
