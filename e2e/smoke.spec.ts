...
test.describe("smoke — routing & hash anchors", () => {
  test("home renders hero and quick facts", async ({ page }) => {
    ...
    await expect(page.getByText("National Shrine · Oklahoma City")).toBeVisible();
    // quick facts grid
    await expect(page.getByText("Grounds Hours")).toBeVisible();
  ...
  test("alias routes — /what-to-see and /grounds-art-architecture", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: /Grounds, Art & Architecture/i })).toBeVisible();
