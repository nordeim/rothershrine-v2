...
| Command | Purpose |
|---|---|
| `pnpm install` | Install deps (Node 20+ for Vite 7, pnpm 11) |
...
| `pnpm test` | Vitest `jsdom` `run` (5 files / 26 tests) |
...
- **`vite.config.ts` `server.watch.ignored`** — ignores `**/skills/**`, `**/dist/**`, `**/playwright-report/**`, `**/test-results/**`, `**/coverage/**` to avoid `ENOSPC` file-watcher limit from `skills` symlink (contains large `.venv`).
