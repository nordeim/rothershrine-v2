...
flowchart TB
  ...
  P --> D[src/data/nav.ts + content.ts + site.ts]
...
│   │   ├── 📄 content.ts    # lifeTimeline, whatToSee, faqs, upcomingEvents (category), givingOptions (name+icon) + images {hero/heroFallback/wheat/atitlan…}
│   │   └── 📄 site.ts       # canonical single source: address, hours (5 keys: grounds/shrineChurch/chapelOfTomb/giftShop/museum), mass (daily/confession/adoration), contact, mapsUrl/mapsEmbedSrc
│   ├── 📂 utils/
